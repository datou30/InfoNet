...
from data import gen_train_data, gen_gauss_xyz, gen_train_data_softrank_weighted, gen_train_data_softrank_weighted_pre
from model.decoder import Decoder 
from model.encoder import Encoder 
from model.infonet import infonet
from model.query import Query_Gen_transformer
...
class InfoNetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        res = gen_train_data_softrank_weighted_pre(batchsize=1, seq_len=seq_len, dim=1, com_num=10, reg=reg).squeeze(0)
        return res
...
class LightningWrapper(lightning.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        dim_list = [1, 2, 5, 10]
        for dim in dim_list:
            eval_acc = gmm_order_eval(self.model, dim, seq_len=5000, number_test=500, iter=self.current_epoch + 1, reg=reg)
    ...
    def train_dataloader(self):
        dataset = InfoNetDataset(total_epoch=4500)
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=15, shuffle=True, num_workers=4, pin_memory=True)
        return dataloader
...
def gmm_order_eval(module, dim, iter, seq_len=2000, number_test=500, reg=0.1):
    ...
    for idx in range(number_test):
        ...
        for row in range(3):
            ###
            tmp = torch.from_numpy(data[:, row])
            tmp = (tmp - torch.min(tmp))/(torch.max(tmp) - torch.min(tmp))
            ###
            tmp = tmp.unsqueeze(0)
            tmp = torchsort.soft_rank(tmp, regularization_strength=reg).squeeze(0).numpy()
    ...
    #print("==============================> order acc is:", accuracy_rate)
...
@hydra.main(config_path='configs', config_name='cfg_softrank_0.1_new.yaml', version_base='1.1')
def main(cfg):

    global latent_dim, latent_num, input_dim, batchsize, seq_len, decoder_query_dim, reg
    ...
    reg = cfg.reg
    # print(f"batchsize: {batchsize}, latent_dim: {latent_dim}, latent_num: {latent_num}, \
    #     input_dim: {input_dim}, seq_len: {seq_len}, decoder_query_dim: {decoder_query_dim}, version: {cfg.version}")
    # gpu_id = cfg.device[HydraConfig.get().job.num % len(cfg.device)]
    # print("gpu id:", gpu_id)
